...
const updateProducto = asyncHandler(async (req, res) => {
    const productoUpdate = await productosmodel.findOneAndUpdate({ sku: req.params.sku }, req.body, { new: true } );
...
const deleteProducto = asyncHandler(async (req, res) => {
    const producto = await productosmodel.findOne({ sku: req.params.sku });
    ...
    await productosmodel.deleteOne(producto);
...
const buscarPorSku = asyncHandler(async (req, res) => {
    ...
    const producto = await productosmodel.findOne({ sku });
